...
namespace uPLibrary.Hardware.Nfc
{
    ...
    public class PN532
    {
        ...
        /// </summary>
        /// <returns>Bytes with firmware information</returns>
        ...
        public async Task<byte[]> InListPassiveTarget(TargetType type)
        {
        ...
        /// <returns></returns>
        public async Task<byte[]> InDataExchange(byte tg, byte[] dataOut)
        {
        ...
        private async Task<byte[]> ExecuteCmd(byte[] cmd)
        {
            //REV - not sure if this lock is needed
            //lock (cmdLock)
            //{
        ...
        {
            byte[] frame = commLayer.ReadNormalFrame();
        ...
        private async Task<bool> SendNormalFrame(byte[] cmd)
        {
            ...
            return await commLayer.SendNormalFrame(frame);
        }
